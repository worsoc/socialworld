 ...
 * Because of being a singleton there exists only one instance of the class.
 * The object access is controlled by a semaphore.
 * Only the owner of the semaphore can use the object.
 * After locking the calculation object the user object gives an attribute array to the calculator.
 * Then there can be a lot of calculations on that attribute array.
 * If no further calculation is necessary the user object gets back the new atribute values.
 * Finally the user releases the calculator. So an other object can take the semaphore
 * and use the calculation object.
 */
...
/**
 * the attribute array that is set by the user
 * and is got back to the user after calculation.
 */
	private AttributeArray attributes;
	
/**
 * a matrix that holds the information how to calculate on the attribute array.
 */	
...
/**
 *  Because of being a singleton the instance is created in a private constructor.	
 */
...
/**
 * The method locks the calculator.
 * The calculator can be locked if and only if there is no other locking object.
 * @param user the locking object / the user of the attribute calculator
 */
...
/**
 * The method releases the calculator.
 * The calculator can be released by the locking object only.
 * @param user the locking object / the user of the attribute calculator
 */
 ...
 * (if and only if  the object is owner of the semaphore).
 * @param attributes
 * @param user
 */
...
/**
 * By calling that method the caller gets the actual state of the attribute array.
 * @param attributes
 */
 ...
 * @param targetAttribute the attribute to modify
 * @param exression the informations how the event changes the attribute
 */
 ...
 * @param matrix the informations how the attributes influence each other.
 */
...
/**
 * The method calculates the change of all attributes.
 * The change information of a matrix is evaluated as often as necessary.
 * The calculation stops when for all attributes the changes are in a range of +/- 1. 
 * @param matrix
 */
...
/**
 * The method calculates the attribute changes.
 * For every matrix element the influence is evaluated
 * and the change is set to the influenced attribute.
 * @return true if the calculation is not finished and 
 * false if the calculation stops.
 */
...
/**
 * The method checks whether the calculation loop continues.
 * The calculation loop stops if for every attribute the value changes by less than 1.
 * @return true if the calculation loop continues and false if teh calculation loop stops
 */
