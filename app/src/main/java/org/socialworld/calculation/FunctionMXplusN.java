	...
	public Value calculate(ValueArrayList arguments) {
			...
			if (this.useFloatingPointCalculation) {
				...
				if (type == Type.floatingpoint)  {
					result = calculation.createValue(Type.floatingpoint, calculateFloatingPoint((float) calculation.createValue(Type.floatingpoint, x.getValue()).getValue()));
					return result;
				}
				if (type == Type.integer) {
					result = calculation.createValue(Type.integer, calculateFloatingPoint((float) calculation.createValue(Type.floatingpoint, x.getValue()).getValue()));
					return result;
				}
