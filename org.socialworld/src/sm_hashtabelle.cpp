...
template <class TypeData> void SM_HashTabelle<TypeData>::insert(THashID ID, TypeData* element)
{
		THash index = hash(ID);
...
template <class TypeData> TypeData* SM_HashTabelle<TypeData>::getElement(THashID ID)
{
...
template <class TypeData> THash SM_HashTabelle<TypeData>::findElement(THashID ID)
{
    ...
    int iDifferenz = 0;
		...
		SM_ListenElement<TypeData>* listenElement = bestPointer(index, ID, bVorwaerts, iDifferenz );
    ...
    if (iDifferenz == 0)
    {
		...
		if (iDifferenz < 0) iDifferenz = - iDifferenz;
...
template <class TypeData> SM_ListenElement<TypeData>* SM_HashTabelle<TypeData>::bestPointer(THash index, THashID ID,
																																						bool& bVorwaerts, int& iDifferenz)
{
