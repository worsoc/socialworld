...
public class TablePoolParseInput extends Table {
	...
	public final  String 	ALL_COLUMNS 		=	" parse_input_id, parse_input_type, parse_input_string ";
	...
	private void selectAllColumns(ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				parse_input_string[row] = rs.getString(3);
	...
	public void insert(int  parse_input_id, int  parse_input_type,  String parse_input_string ) {
		String statement;
		...
		if ((parse_input_id > 0) & (parse_input_type > 0)  & (parse_input_string.length() > 0)) {
			...
			statement 	= "INSERT INTO swpool_parseinputs (" + ALL_COLUMNS + ") VALUES (" + 
					parse_input_id + ", " + parse_input_type + ", '" + parse_input_string + "')";
